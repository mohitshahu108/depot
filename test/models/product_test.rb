...
class ProductTest < ActiveSupport::TestCase
 ...
 test "product attributes must not be empty" do
  ...
  test "product price must be positive" do
    product = Product.new(title: "My Book", description: "yyy")
  ...
  def new_product(file_name, content_type)
    Product.new(
      title: "My Book title",
      description: "yyy",
      price: 1,
    ).tap do |product| 
      product.image.attach(io: File.open("test/fixtures/files/#{file_name}"), filename: "#{file_name}", content_type: content_type)
    end
  end
